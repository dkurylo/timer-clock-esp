...
const uint16_t eepromTimerSetupResetTimeIndex = eepromBrightnessSteepnessCoefficientIndex + 1;
...
void loadEepromData() {
  ...
  if( eepromFlashDataVersion != 255 && eepromFlashDataVersion == EEPROM_FLASH_DATA_VERSION ) {
    ...
    readEepromUint8Value( eepromTimerRememberLastInputTimeIndex, timerRememberLastInputTimeMinutes, true );
    readEepromUint8Value( eepromTimerDeltaPressIndex, timerDeltaPressMinutes, true );
    readEepromUint8Value( eepromTimerDeltaTurnIndex, timerDeltaTurnSeconds, true );
    if( timerDeltaTurnSeconds == 0 ) timerDeltaTurnSeconds = 60;
    readEepromUint8Value( eepromTimerBlinkingTimeIndex, alarmBlinkingTimeMinutes, true );
    readEepromUint8Value( eepromTimerBeepingTimeIndex, alarmBeepingTimeSeconds, true );
    ...
    if( timerSetupResetTimeSeconds < 5 ) timerSetupResetTimeSeconds = 30;
    writeEepromUint8Value( eepromTimerSetupResetTimeIndex, timerSetupResetTimeSeconds );
    writeEepromUint8Value( eepromTimerRememberLastInputTimeIndex, timerRememberLastInputTimeMinutes );
    writeEepromUint8Value( eepromTimerDeltaPressIndex, timerDeltaPressMinutes );
    if( timerDeltaTurnSeconds == 0 ) timerDeltaTurnSeconds = 60;
    writeEepromUint8Value( eepromTimerDeltaTurnIndex, timerDeltaTurnSeconds );
    writeEepromUint8Value( eepromTimerBlinkingTimeIndex, alarmBlinkingTimeMinutes );
    writeEepromUint8Value( eepromTimerBeepingTimeIndex, alarmBeepingTimeSeconds );
...
"<html>"
  "<head>"
    ...
    "<style>"
      ...
      "#gc{width:100%;aspect-ratio:2/1;border:1px solid #aaa;display:flex;flex-direction:column;margin:0.2em 0 1.5em 0.8em;}" //graph start
      "#gw{position:relative;width:100%;flex:1;font-size:0.65em;}"
      "#gr,#yg,#xg,.ym,.yl,.xm,.xl,.sm,.cvm,.cvl{position:absolute;pointer-events:none;}"
      "#gr,#yg,#xg{inset:0;}"
      "#gr{display:flex;align-items:flex-end;pointer-events:auto;}"
      ".gbar{background:#4caf50;}"
      ".ym{left:0;right:0;height:1px;border-top:1px dotted RGBA(255,255,255,0.35);}"
      ".xm{top:0;bottom:0;width:0;border-left:1px dotted RGBA(255,255,255,0.35);}"
      ".yl,.xl,.cvl{font-size:0.75em;color:#aaa;text-align:right;}"
      ".yl{left:-2em;top:50%;transform:translateY(-50%);width:1.5em;}"
      ".xl{bottom:-2.5em;transform:translateY(-1.5em);width:3em;}"
      ".xlr{rotate:-90deg;letter-spacing:0.05em;}"
      ".sm{top:0;bottom:0;width:0;border-left:1px solid RGBA(255,255,255,0.45);z-index:4;}"
      ".cvm,.cvl{opacity:0.9;transition:left 0.3s ease;}"
      ".cvm{top:0;bottom:0;width:1px;background:#fe5;z-index:5;}"
      ".cvl{left:0;top:0.2em;color:#fe5;z-index:6;}" //graph end
    ...
    "<div class=\"wrp\">"
      "<h2>"
        ...
        "<div style=\"line-height:0.5;\">"
          ...
          "<div class=\"lnk\" style=\"font-size:50%;\"><a id=\"fwup\" href=\"https://github.com/dkurylo/timer-clock-esp\" target=\"_blank\">GitHub</a></div>"
        "</div>"
      "</h2>";
...
"<script>"
  ...
  "function mnt(id){"
    ...
    "}else{"
      ...
      "mntIds.add(id);"
    "}"
    "if(isAdd||(!mntIds.size&&mntTimerId)){"
      "clearTimeout(mntTimerId);"
      "mntTimerId=null;"
    "}"
    "if(isAdd){"
      "mnf();"
  ...
  "let graph={" //graph start
    ...
    "drawG(){"
      ...
      "for(let s of this.segs){"
        ...
        "d.className=\"gbar\";"
        "d.style.width=((s.t-s.f)/(this.gp.smax-this.gp.smin)*100)+\"%\";"
    ...
    "drawX(){"
      ...
      "for(let s of this.segs){"
        ...
        "let x=((s.f-this.gp.smin)/(this.gp.smax-this.gp.smin)*100)+\"%\";"
        "let l=document.createElement(\"div\");l.className=\"xm\";l.style.left=x;"
        "let t=document.createElement(\"div\");t.className=\"xl xlr\";t.style.left=x;t.textContent=s.f;"
        "g.appendChild(l);g.appendChild(t);"
      "}"
      "let mn=document.createElement(\"div\"),mx=document.createElement(\"div\");"
      "mn.className=mx.className=\"xm sm\";"
      "let r=this.gp.smax-this.gp.smin;"
      "mn.style.left=((this.gp.ns-this.gp.smin)/r*100)+\"%\";"
      "mx.style.left=((this.gp.ds-this.gp.smin)/r*100)+\"%\";"
      "g.appendChild(mn);"
      "g.appendChild(mx);"
    "},"
    "draw(isInit){"
    ...
    "drawCurVal(cv){"
      "let p=this.gp;"
      "p.cv=cv;"
      "let elm=document.getElementById(\"cvm\");"
      "let ell=document.getElementById(\"cvl\");"
      "if(p.cv==-1){"
        "if(ell)ell.remove();"
        "if(elm)elm.remove();"
        "return;"
      ...
      "if(!elm){"
        ...
        "elm.id=\"cvm\";"
        "elm.className=\"cvm\";"
        "document.getElementById(\"xg\").appendChild(elm);"
      "}"
      "elm.style.left=pct+\"%\";"
      "if(!ell){"
        ...
        "elm.appendChild(ell);"
      "}"
      "ell.style.left=\"calc(\"+pct+\"% - \"+(pct<90?-0.4:2.6)+\"em)\";"
...
"<div class=\"fx ft\">"
  "<span>"
    "<div class=\"stat\">"
      "<span class=\"btn\" onclick=\"this.classList.toggle('on');mnt(1);\">Яскравість</span>"
...
void handleWebServerPost() {
  ...
  if( timerSetupResetTimeSecondsReceivedPopulated && timerSetupResetTimeSecondsReceived != timerSetupResetTimeSeconds ) {
    ...
    writeEepromUint8Value( eepromTimerSetupResetTimeIndex, timerSetupResetTimeSecondsReceived );
  }
  ...
  if( timerRememberLastInputTimeReceivedPopulated && timerRememberLastInputTimeReceived != timerRememberLastInputTimeMinutes ) {
    ...
    writeEepromUint8Value( eepromTimerRememberLastInputTimeIndex, timerRememberLastInputTimeReceived );
  }
  ...
  if( timerDeltaPressReceivedPopulated && timerDeltaPressReceived != timerDeltaPressMinutes ) {
    ...
    writeEepromUint8Value( eepromTimerDeltaPressIndex, timerDeltaPressReceived );
  }
  ...
  if( timerDeltaTurnReceivedPopulated && timerDeltaTurnReceived != timerDeltaTurnSeconds ) {
    ...
    writeEepromUint8Value( eepromTimerDeltaTurnIndex, timerDeltaTurnReceived );
  }
  ...
  if( timerBlinkingTimeReceivedPopulated && timerBlinkingTimeReceived != alarmBlinkingTimeMinutes ) {
    ...
    writeEepromUint8Value( eepromTimerBlinkingTimeIndex, timerBlinkingTimeReceived );
  }
  ...
  if( timerBeepingTimeReceivedPopulated && timerBeepingTimeReceived != alarmBeepingTimeSeconds ) {
    ...
    writeEepromUint8Value( eepromTimerBeepingTimeIndex, timerBeepingTimeReceived );
  }
